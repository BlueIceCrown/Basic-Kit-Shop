...
world.afterEvents.itemUse.subscribe((data) => {
    ...
    if (!kit)
        return;
    if (!takeOne(player, itemId))
        return;
    giveKit(player, kit);
});
function giveKit(player, kit) {
...
export function assembleItem(itemData, fallbackEnchants = [], displayName) {
...
export function clearItem(player, itemId, decrement) {
    const inventory = player.getComponent('inventory').container;
    const clearSlots = [];
    for (let i = 0; i < 36; i++) {
...
export function takeOne(player, typeId) {
